'''
Handle equality operations on querys. a = b, etc...
'''
...
class _Operand:
    ...
    def serialize(self, \
        ...
        expression_attribute_values: Dict[str, str]) -> str:
        '''serialize the operand to a string'''
...
class _ListAppendOperand(_Operand):
    '''
    A base class for Operands that can be used in the list_append
    function for the SET update action.
    '''
...
class Increment(_Operand):
    ...
    def __init__(self, lhs: '_Operand', rhs: '_Operand') -> None:
        ...
        super(Increment, self).__init__(lhs, rhs) # pylint: disable=super-with-arguments
...
class Decrement(_Operand):
    ...
    def __init__(self, lhs: _Operand, rhs: _Operand) -> None:
        lhs._type_check(NUMBER_SHORT)
        rhs._type_check(NUMBER_SHORT)
        super(Decrement, self).__init__(lhs, rhs) # pylint: disable=super-with-arguments

# pylint: disable=too-few-public-methods
class ListAppend(_Operand):
    '''
    ListAppend is a special operand that represents the list_append
    function for the SET update action.
    '''
    ...
    def __init__(self, list1: _Operand, list2: _Operand):
        ...
        super(ListAppend, self).__init__(list1, list2) # pylint: disable=super-with-arguments


class IfNotExists(_NumericOperand, _ListAppendOperand):
    '''
    IfNotExists is a special operand that represents the if_not_exists
    function for the SET update action.
    '''
    ...
    def __init__(self, path: _Operand, value: Any) -> None:
        ...
        super(IfNotExists, self).__init__(path, value) # pylint: disable=super-with-arguments
...
class Value(_NumericOperand, _ListAppendOperand):
    ...
    def __init__(self, value: Any, attribute: Optional['Attribute'] = None) -> None:
        ...
        if isinstance(value, dict) and len(value) == 1 \
            and list(value.keys())[0] in SHORT_ATTR_TYPES:
        ...
        super(Value, self).__init__(value) # pylint: disable=super-with-arguments
...
class Path(_NumericOperand, _ListAppendOperand):
    ...
    def __init__(self, attribute_or_path: _PathOrAttribute) -> None:
        ...
        super(Path, self).__init__(get_path_segments(path)) # pylint: disable=super-with-arguments

    @property
    def path(self) -> Any:
        '''return the path of itself'''
        return self.values[0]
    ...
    def __getitem__(self, item: Union[int, str]) -> 'Path':
        ...
        This is intended since the nested element is not the same
        attribute as ``self``.
        '''
    ...
    def add(self, *values: Any) -> AddAction:
        '''
        Returns an update action that appends the given values
        to a set or mathematically adds a value to a number
        '''
    ...
    def delete(self, *values: Any) -> DeleteAction:
        '''
        Returns an update action that removes the given
    ...
    def _serialize_value(self, value, placeholder_names, expression_attribute_values):
        '''serialize the value for a condition'''
        return substitute_names(value, placeholder_names)

    def _to_value(self, value: Any) -> Value:
        '''turn the operand into a value for queries'''
        return Value(value, attribute=self.attribute)

    def __str__(self) -> str:
        '''
        Quote the path to illustrate that any dot characters are not
    ...
    def __repr__(self) -> str:
        '''implement string representation of this class'''
        return "Path({})".format(self.path)

    @staticmethod
    def _quote_path(path):
        '''take a guess at what this does. IDK'''
